...
import { formatList, formatEndDate } from './work-package-summary';
...
describe('Formatting lists tests', () => {
  ...
  test('Formatting Rules', () => {
    expect(
      formatList(exampleWorkPackage1.rules, (str: string): string => {
        return str;
      })
    ).toBe('EV3.5.2');
    expect(
      formatList(exampleWorkPackage2.rules, (str: string): string => {
        return str;
      })
    ).toBe('T12.3.2, T8.2.6');
    expect(
      formatList(exampleWorkPackage3.rules, (str: string): string => {
        return str;
      })
    ).toBe('EV1.4.7, EV6.3.10');
  });

  test('for other case', () => {
    expect(
      formatList([1, 2, 3, 4], (num: number): string => {
        return '' + num;
      })
    ).toBe('1, 2, 3, 4');
    expect(
      formatList([], (num: number): string => {
        return '' + num;
      })
    ).toBe('');
    expect(
      formatList([1], (num: number): string => {
        return '' + num;
      })
    ).toBe('1');
  });
...
describe('Formatting end date tests', () => {
  test('with dummy data', () => {
    expect(formatEndDate(exampleWorkPackage1.startDate, exampleWorkPackage1.duration)).toBe(
      '1/22/2021'
    );
    expect(formatEndDate(exampleWorkPackage2.startDate, exampleWorkPackage2.duration)).toBe(
      '2/26/2021'
    );
    expect(formatEndDate(exampleWorkPackage3.startDate, exampleWorkPackage3.duration)).toBe(
      '1/15/2021'
    );
  });
...
describe('Rendering Work Packagae Summary Test', () => {
  test('Rendering example 1', () => {
    ...
    expect(
      screen.getByText(`Dependencies:${formatList(wp.dependencies, wbsPipe)}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        `${formatList(wp.rules, (str: string): string => {
          return str;
        })}`
      )
    ).toBeInTheDocument();
