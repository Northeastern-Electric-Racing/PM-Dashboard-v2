...
describe('Formatting lists tests', () => {
  ...
  test('Formatting Rules', () => {
    expect(
      listPipe(exampleWorkPackage1.rules, (str: string): string => {
        return str;
      })
    ).toBe('EV3.5.2');
    expect(
      listPipe(exampleWorkPackage2.rules, (str: string): string => {
        return str;
      })
    ).toBe('T12.3.2, T8.2.6');
    expect(
      listPipe(exampleWorkPackage3.rules, (str: string): string => {
        return str;
      })
  ...
  test('for other case', () => {
    expect(
      listPipe([1, 2, 3, 4], (num: number): string => {
        return '' + num;
      })
    ).toBe('1, 2, 3, 4');
    expect(
      listPipe([], (num: number): string => {
        return '' + num;
      })
    ).toBe('');
    expect(
      listPipe([1], (num: number): string => {
        return '' + num;
      })
    ).toBe('1');
  });
